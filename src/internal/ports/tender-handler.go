...
func (h *TenderHandler) GetTenderByFilter(w http.ResponseWriter, r *http.Request) {
	...
	if filters.Bind(r) != nil {

		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Ошибка запроса")))
	...
	if err != nil {
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Ошибка сервера")))
...
func (h *TenderHandler) CreateTender(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, consts.UserIsNotExistError) {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Пользователя %s не существует или организации  id = %s", req.CreatorUsername, req.OrganizationId)))
	...
	if errors.Is(err, consts.UserHasNoRights) {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Пользователь %s нет прав ", req.CreatorUsername)))
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Ошибка создания тендера")))
		w.WriteHeader(http.StatusInternalServerError)
...
func (h *TenderHandler) GetTendersByUsername(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, consts.UserIsNotExistError) {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Пользователя %s не существует", filters.Username)))
		w.WriteHeader(http.StatusUnauthorized)
...
func (h *TenderHandler) GetTenderStatusById(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, consts.UserHasNoRights) {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("У пользователя %s нет прав", filters.Username)))
		w.WriteHeader(http.StatusForbidden)
	...
	if errors.Is(err, consts.TenderIsNotExistError) {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Тендер не существует")))
...
func (h *TenderHandler) UpdateTenderVersionRollback(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, consts.TenderIsNotExistError) {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Тендера %s не существует", filters.TenderId)))
		w.WriteHeader(http.StatusNotFound)
	...
	if errors.Is(err, consts.VersionIsNotExistError) {
		...
		json.NewEncoder(w).Encode(response.NewError(fmt.Sprintf("Версия %s не существует", filters.Version)))
		w.WriteHeader(http.StatusNotFound)
