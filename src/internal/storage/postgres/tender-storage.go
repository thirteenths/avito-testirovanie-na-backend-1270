...
const createTenderVersionQuery = `INSERT INTO version (name, description, service_type, number, tender_id) 
...
func (s *tenderStorage) CreateTender(tender domain.Tender) (domain.Tender, error) {
	...
	err = tx.QueryRow(
		createTenderVersionQuery,
		tender.Name,
		tender.Description,
		tender.ServiceType,
		1,
		tender.Id,
	).Scan(&tender.VersionId)
	if err != nil {
...
const getTenderByIdQuery = `SELECT tender.id, name, description, service_type, status, organization_id, 
       version.id, number, created_at 
FROM tender  JOIN version ON tender.id = version.tender_id 
WHERE tender.id = $1 AND number=(SELECT MAX(number) FROM version WHERE tender_id =$1)`
...
const updateTenderVersionByIdQuery = `INSERT INTO version  (name, description, service_type, number, tender_id) 
VALUES ($1, $2, $3, $4, $5) RETURNING id`
...
const getTenderVersionByIdQuery = `SELECT id, name, description, service_type 
FROM version  WHERE tender_id=$1 AND number = $2`
...
const checkVersionTenderIsExistQuery = `SELECT EXISTS (
SELECT version FROM version  WHERE number = $1 AND tender_id = $2)`
